    ...
    def __init__(self, json_file, tokenizer, max_len=512):
        ...
        for i, item in enumerate(self.raw_data):
            try:
                ...
                # Ensure context and answer are strings and normalize them
                if not isinstance(context, str) or not isinstance(answer, str):
                    print(f"Skipping item {i} with invalid types - question: {question}, context: {context}, answer: {answer}")
                    continue

                # Normalize context and answer by removing extra whitespace
                ...
                if start_idx == -1 or end_idx == -1:
                    print(f"Skipping item {i} with answer not found in context - question: {question}, context: {context}, answer: {answer}")
                ...
                if start_positions is None or end_positions is None:
                    print(f"Skipping item {i} with positions not found - question: {question}, context: {context}, answer: {answer}")
                ...
                print(f"Error processing item {i}: {item}")
                print(f"Exception: {e}")
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            try:
                ...
                print(f"Error during training at epoch {epoch + 1}, batch: {batch}")
                print(f"Exception: {e}")
